...
public class DiseaseSearchDao implements SearchDao {

    public static final String DISEASE_NAME_QUERY_NAME = "name";
    public static final String DISEASE_PLANT_ID_QUERY_NAME = "plantId";
    ...
    public String query(boolean applySizeFilters) throws CustomException {
        String query = "SELECT * FROM " + DISEASE_TABLE + " WHERE ";

        if (name != null && isNotEmpty(name.getCondition()) && isNotEmpty(name.getValue())) {
            query += addStringFilter(name, NAME, DISEASE_NAME_QUERY_NAME, map);
        }

        if (plantId != null && isNotEmpty(plantId.getCondition()) && isValidInteger(plantId.getValue())) {
            query += addIntegerFilter(plantId, PLANT_ID, DISEASE_PLANT_ID_QUERY_NAME, map);
        }
